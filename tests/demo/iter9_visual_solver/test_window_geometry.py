...
class WindowGeometryTests(unittest.TestCase):
    ...
    def test_resize_geometry_preserves_larger_requested_window_size(self):
        ...
        self.assertEqual(geometry.window_width, 1920)
        self.assertEqual(geometry.window_height, 1080)
        self.assertGreater(geometry.board_scale, 1.0)
        self.assertGreater(geometry.board_draw_rect.width, 800)
        self.assertEqual(geometry.status_panel_rect.width, 537)
        self.assertEqual(geometry.status_panel_rect.x, 1367)
        self.assertEqual(geometry.header_rect.width, 1920)
        self.assertLess(geometry.board_draw_rect.x + geometry.board_draw_rect.width, geometry.status_panel_rect.x)
    ...
    def test_status_panel_and_source_preview_rects_are_inside_geometry(self):
        ...
        self.assertGreaterEqual(preview.x, panel.x)
        self.assertGreaterEqual(preview.y, panel.y)
        self.assertLessEqual(preview.x + preview.width, panel.x + panel.width)
        self.assertLessEqual(preview.y + preview.height, panel.y + panel.height)
    ...
    def test_calculate_window_placement_centers_x_when_enabled(self):
        ...
        self.assertEqual(placement.x, 310)
